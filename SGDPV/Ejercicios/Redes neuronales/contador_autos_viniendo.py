...
# Querés contar estos (ajustalo si querés)
...
class IoUTracker:
    def __init__(self, iou_match_thresh=0.25, max_missed=12):
...
# ---------------------------
def main():
    # Usá tu video original acá (ya no hace falta uno "derecha")
    ...
    while True:
        ...
        # ---------------------------
        x0 = ancho // 2
        roi = frame[:, x0:ancho]                 # solo derecha
        roi_h, roi_w = roi.shape[:2]

        # (opcional) dibujar el ROI en rojo para verlo
        cv2.rectangle(frame, (x0, 0), (ancho - 1, altura - 1), (0, 0, 255), 2)

        # línea de conteo (la tuya)
        posicionY_linea = int(altura / 2) + 100
        ...
        # factores respecto al ROI (no al frame)
        ...
        for i in range(detections.shape[2]):
            ...
            # IMPORTANTÍSIMO: pasar a coords del frame completo (sumando offset x0)
            x1 += x0
            x2 += x0
        ...
        # dibujar línea
        cv2.line(frame, (x0, posicionY_linea), (ancho - 1, posicionY_linea), (255, 0, 0), 4)
        ...
        cv2.imshow("Contador de autos (ROI derecha)", frame)
