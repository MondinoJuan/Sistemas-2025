...
def load_and_process_model(model_path, marker_size=0.05):
    ...
    try:
        ...
        print(f"Tipo de objeto cargado: {type(loaded)}")
        
        # Si es una Scene (común con GLTF/GLB), extraer el primer mesh
        if isinstance(loaded, trimesh.Scene):
            print(f"Escena con {len(loaded.geometry)} geometrías")
            
            # Listar todas las geometrías
            for name, geom in loaded.geometry.items():
                print(f"  - {name}: {type(geom)}")
            
            # Obtener el primer mesh válido
            model = None
            for name, geom in loaded.geometry.items():
                if isinstance(geom, trimesh.Trimesh):
                    model = geom
                    print(f"Usando geometría: {name}")
                    break
            
            if model is None:
                print("No se encontró ningún mesh válido en la escena")
                return create_default_building()
                
        elif isinstance(loaded, trimesh.Trimesh):
            model = loaded
            print("Mesh único cargado directamente")
        else:
            print(f"Tipo no soportado: {type(loaded)}")
            return create_default_building()
        
        print(f"Modelo procesado: {len(model.vertices)} vértices, {len(model.faces)} caras")
        ...
        # Aplicar color naranja forzado si no hay colores
        if not hasattr(model.visual, 'face_colors') or len(model.visual.face_colors) == 0:
            print("⚠ Aplicando color naranja manualmente (forzado)")
            orange = np.array([255, 101, 0, 255], dtype=np.uint8)
            model.visual.face_colors = np.tile(orange, (len(model.faces), 1))
...
def get_face_color(model, face_idx):
    ...
    try:
        ...
        if hasattr(model.visual, 'face_colors') and len(model.visual.face_colors) > face_idx:
            color = model.visual.face_colors[face_idx][:3]
...
# Función para combinar múltiples meshes de una escena
def combine_scene_meshes(scene):
    """Combinar todos los meshes de una escena en uno solo"""
    meshes = []
    ...
    if len(meshes) == 0:
        print("No se encontraron meshes en la escena")
        return None
    elif len(meshes) == 1:
        return meshes[0]
    else:
        print(f"Combinando {len(meshes)} meshes...")
...
def main(model_path=None):
    ...
    print("Muestra un marcador ArUco al de la cámara")
...
if __name__ == "__main__":
    ...
    # OPCIÓN 1: Archivo GLTF/GLB
    main("C:/Users/Admin/Desktop/Estudios/Sistemas-2025/SGDPV/Ejercicios/ArUco/Archivos3D/scene.gltf")
    #main("C:/Users/Admin/Desktop/Estudios/Sistemas-2025/SGDPV/Ejercicios/ArUco/Archivos3D/AR-Code-1683008113119.glb")
    
    # OPCIÓN 2: Archivo OBJ
    #main("C:/Users/Admin/Desktop/Estudios/Sistemas-2025/SGDPV/Ejercicios/ArUco/Archivos3D/Safety_Cone.obj")
    
    # OPCIÓN 3: Para usar modelo por defecto, comenta la línea de arriba y descomenta esta:
    # main()