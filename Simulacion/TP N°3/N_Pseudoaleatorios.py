...
# Evalúo longitud de período
# Test de duplicados (Birthday spacing test)
def test_birthday_spacing(numeros, bins=10, alpha=0.05):
    indices = {}
    distancias = []

    for i, num in enumerate(numeros):
        if num in indices:
            distancias.append(i - indices[num])
        indices[num] = i
    ...
    # Esto convierte las distancias a una escala comparable, de modo que no dependan del tamaño total de los datos.
    distancias_normalizadas = np.array(distancias) / max(distancias)

    # Aplicar chi-cuadrado al histograma de distancias normalizadas
    # Este test compara la distribución de las distancias normalizadas con una distribución uniforme 
    # (que es la esperada si las distancias entre duplicados fueran aleatorias).
    chi2, p_value = test_chi_square(distancias_normalizadas, bins, alpha)
    ...
    # Este test evalúa si las distancias entre duplicados siguen una distribución uniforme, 
    # que es lo que se esperaría de un generador de números aleatorios. Si los duplicados están demasiado agrupados o demasiado dispersos, 
    # esto podría indicar un problema en el generador de números aleatorios.
